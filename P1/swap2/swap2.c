...
void *swap(void *ptr)
{
	...
	while(1) {
		...
		pthread_mutex_lock(args->mutex1); /*No haria falta proteger las iteraciones porque no es una variable compartida*/ 
		if(args->iterations == 0){
			pthread_mutex_unlock(args->mutex1);
			break;
		}
		(args->iterations)--;
		pthread_mutex_unlock(args->mutex1); /*No haria falta proteger las iteraciones porque no es una variable compartida*/ 
