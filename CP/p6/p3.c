...
int main (int argc, char * argv[] )
{
    ...
    buffer = malloc((X_RESN*Y_RESN)*sizeof(int));
        ...
        for(j=0; j < Y_RESN; j++) {
          ...
          if (k >= maxIterations) buffer[i+j - rank*X_RESN*(Y_RESN/(numprocs))] = 0;
          else buffer[i+j - rank*X_RESN*(Y_RESN/(numprocs))] = k;

          //if(rank = 1) printf("%d\n", i+j - rank*X_RESN*(Y_RESN/(numprocs)));
          //(Y_RESN*X_RESN*rank/numprocs)
          //if (rank==1) printf("%d\n",k);
        }
		...
		MPI_Gather(buffer,(X_RESN*Y_RESN)/numprocs,MPI_INT,res,(X_RESN*Y_RESN)/numprocs,MPI_INT,0,MPI_COMM_WORLD);
